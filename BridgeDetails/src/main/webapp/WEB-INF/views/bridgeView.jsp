	...
	xmlhttp.onreadystatechange = function(){
		if(xmlhttp.readyState ==4 && xmlhttp.status == 200)
			...
			for (i =0; i< bridge.length;i++)
				{
				main+="<tr><td>"+bridge[i].name+"</td><td>"+bridge[i].length+"</td><td>"+bridge[i].height+"</td><td>"+bridge[i].width+"</td><td>"+bridge[i].gpsLocation+"</td></tr>";
...
<form:form name="bridge" id="bridge" method="post" modelAttribute="formData">
        <table>
        <tr>
        <td colspan="2"><h1>Bridge Details</h1></td>
        </tr>
            <tr>
